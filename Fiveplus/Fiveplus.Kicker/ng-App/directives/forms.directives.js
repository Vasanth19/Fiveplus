...
var compareTo = function ()
{
    return {
       ...
       //     scope.otherModelValue = attrs['compare-to'];
...
myDirectives.directive('ncgInput', ['$compile', '$http', '$templateCache', '$interpolate', function ($compile, $http, $templateCache, $interpolate) {
    return {
        restrict: "E",
        replace: true,
        ...
        compile: function compile(element, attrs) {
            ...
            var template = attrs.template || 'ncgInput';   // Default to the simple input if none given, accepts ncgTextarea
            var getFieldElement = $http.get('/ng-App/directives/templates/' + template + '.html', { cache: $templateCache }).then(function (response) {
                var newElement = angular.element(response.data);
                var inputElement = angular.element(newElement.find('input')[0] || newElement.find('textarea')[0]);
